...
public class AiDialogueClient {
    ...
    /**
     * 세션 종료 직후 호출. AI 서버 {@code POST /dialogue/embed-session} 으로 turns 를 보내 환아별 벡터 DB 에 적재한다.
     *
     * <p>{@code @Async} 빈 외부 호출 시점에 Spring proxy 가 작동하여 별도 스레드로 위임. 호출자(세션 종료 트랜잭션) 는 즉시 반환된다.
     */
    ...
    public void embedSessionAsync(DialogueSession session, List<DialogueTurn> turns) {
        ...
        if (turns == null || turns.isEmpty()) {
            log.debug("Empty turns — skip embed for session={}", session.getId());
            return;
        }

        Map<String, Object> body =
                Map.of(
                        "patient_profile_id", session.getPatientProfile().getId(),
                        "session_id", session.getId(),
                        "npc_name", session.getNpcName().name(),
                        "turns", turns.stream().map(AiDialogueClient::toTurnPayload).toList());
        ...
        try {
            ...
            log.warn(
                    "AI embed-session call failed session={} reason={}",
                    session.getId(),
                    e.getMessage());
        }
    }
