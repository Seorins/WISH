 ...
 *
 * <p>핵심 시나리오: 세션 생성 → 첫 장면 구성 / 턴 라우팅 / 조기 종료 / maxSteps 도달 / 세션 종료 / 이미 종료된 세션 거부 / NPC 미지원 / 권한 /
 * 인증.
 */
...
class DialogueControllerIntegrationTest extends IntegrationTestSupport {
    ...
    @Test
    @DisplayName("마을 주민 NPC 는 본 단계에서 미지원 — DL-005 (501)")
    void createSession_villager_returnsNotSupported() throws Exception {
        TestUser user = setupUserWithProfile("villager@example.com", "villager-user");
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.code").value("DL-005"));

        assertThat(dialogueSessionRepository.count()).isZero();
    }
    ...
    @Test
    @DisplayName("세션 시작 — 인증 필요")
    void createSession_requiresAuthentication() throws Exception {
        mockMvc.perform(
                        post("/dialogue/sessions")
                                .contentType(MediaType.APPLICATION_JSON)
    ...
    void submitTurn_moodWorried_routesToWorrySource() throws Exception {
        ...
        mockMvc.perform(
                        post("/dialogue/sessions/{id}/turns", sessionId)
                                .header("Authorization", "Bearer " + user.token())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(turnBody("mood_worried", "걱정돼요", 2)))
    ...
    void submitTurn_actionBreathe_endsSessionEarly() throws Exception {
        ...
        submitTurn(user, sessionId, "mood_okay", "괜찮아요", 0);
        ...
        mockMvc.perform(
                        post("/dialogue/sessions/{id}/turns", sessionId)
                                .header("Authorization", "Bearer " + user.token())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(turnBody("action_breathe", "숨을 천천히 쉬어요", 0)))
                .andExpect(status().isOk())
    ...
    void submitTurn_maxStepsReached_endsSession() throws Exception {
        ...
        submitTurn(user, sessionId, "mood_worried", "걱정돼요", 2); // step 0
        submitTurn(user, sessionId, "worry_pain", "아픈 게 걱정돼요", 3); // step 1
        ...
        mockMvc.perform(
                        post("/dialogue/sessions/{id}/turns", sessionId)
                                .header("Authorization", "Bearer " + user.token())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(turnBody("support_medical", "선생님께 말할래요", 0)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.nextScene.shouldEndSession").value(true));
    ...
    void submitTurn_alreadyFinished_returnsConflict() throws Exception {
        ...
        mockMvc.perform(
                        post("/dialogue/sessions/{id}/turns", sessionId)
                                .header("Authorization", "Bearer " + user.token())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(turnBody("mood_okay", "괜찮아요", 0)))
    ...
    @Test
    @DisplayName("세션 종료 — closingLines 만 반환, 마음엽서/분석 결과 없음")
    void finish_returnsClosingLinesOnly() throws Exception {
    ...
    @Test
    @DisplayName("REST_TODAY 종료는 다른 closingLines 사용")
    void finish_restToday_usesRestClosingLines() throws Exception {
    ...
    @Test
    @DisplayName("세션 상세 조회 — 메타 + 모든 턴 stepIndex 오름차순")
    void getDetail_returnsSessionAndTurnsOrdered() throws Exception {
        TestUser user = setupUserWithProfile("detail@example.com", "detail-user");
        long sessionId = startSession(user, "YEONGCHEOL");
        submitTurn(user, sessionId, "mood_worried", "걱정돼요", 2);
        submitTurn(user, sessionId, "worry_pain", "아픈 게 걱정돼요", 3);
        finishSession(user, sessionId, "COMPLETED");
    ...
    private void submitTurn(
            TestUser user, long sessionId, String choiceIntentId, String text, int intensity)
            throws Exception {
        mockMvc.perform(
                        post("/dialogue/sessions/{id}/turns", sessionId)
                                .header("Authorization", "Bearer " + user.token())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(turnBody(choiceIntentId, text, intensity)))
                .andExpect(status().isOk());
    }
    ...
    private String turnBody(String choiceIntentId, String text, int intensity) {
        return """
                {
                  "selectedChoice": {
                    "choiceIntentId": "%s",
                ...
                """
                .formatted(choiceIntentId, text, intensity);
    }
