...
세션·진척도·벨트 이력처럼 **환자 1명의 누적 데이터를 조회하는 컬렉션** 은 `?patientProfileId=` 를 필수 파라미터로 받는다 (4.1 표의 마지막 4개). 의도:

- **다중 환자 / 의료진 확장 대비 (6.1, 6.2)**: 1:N 으로 열리는 시점에 API 시그니처를 바꾸지 않고도 그대로 동작. 보호자 1:1 인 현재는 redundant 하지만 미래 변경 비용을 미리 흡수한다.
- **권한 의도의 가시성**: service 가 `(currentUser, patientProfileId)` 둘 다 받아 owner 검증을 명시적으로 호출 (`PatientProfileService.findOwnedOrThrow`). 자동 lookup 으로 숨기지 않음.
