...
# 주간 리포트 요약은 보호자가 한눈에 보는 종합 코멘트라 품질이 중요 → Opus 기본.
# DIALOGUE_CHAT_MODEL(영철 채팅용 Haiku) 과는 별개로 운용한다.
...
FALLBACK_RESPONSE = ReportSummaryResponse(
    summary=[
...
async def summarize_weekly_report(request: ReportSummaryRequest) -> ReportSummaryResponse:
    ...
    try:
        ...
        logger.error(
            "[ReportSummary] Claude HTTP %d (patient=%d)",
