 ...
 * 우측: 운동 종류별 동작 리스트(클릭 시 좌측 영상 교체) + 지난 영상(빈 박스).
 *
 * 통계/지난영상은 BE me-sessions 엔드포인트(`GET /me/exercise-sessions`)와
 * `ExerciseSessionMotionResult.videoUrl` 컬럼이 추가되면 실데이터로 채움.
 */
...
function VideoCard({ motion }: { motion: ExerciseMotion }) {
...
function StatsCard({ motion }: { motion: ExerciseMotion }) {
