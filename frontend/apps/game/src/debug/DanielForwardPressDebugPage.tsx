...
type DanielForwardPressApiResponse = {
  motion_id: string
  state: string
  accuracy: number
...
function DanielForwardPressDebugPage() {
  ...
  const [result, setResult] = useState<DanielForwardPressApiResponse | null>(null)
  ...
  const startTracking = async () => {
    ...
    try {
      ...
      resetEvaluatorState()
      setRunning(true)
      setStatus('Daniel forward press test running')
      ...
      const loop = async () => {
        ...
        if (pose && !requestInFlightRef.current) {
          ...
          try {
            const response = await fetch(`${aiBaseUrl}/gymnastics/daniel-forward-press/evaluate`, {
              method: 'POST',
            ...
            const payload = (await response.json()) as DanielForwardPressApiResponse
  ...
  return (
    <div style={pageStyle}>
      <div style={layoutStyle}>
        <section>
          <h1 style={{ margin: 0, fontSize: '32px' }}>Daniel Forward Press AI Debug</h1>
          <p style={{ marginTop: '8px', color: '#4d5b71' }}>
            Capture pose landmarks from the webcam, send them to the `daniel-forward-press/evaluate`
            API, and inspect hold-based forward press evaluation in real time.
          </p>
...
export default DanielForwardPressDebugPage
