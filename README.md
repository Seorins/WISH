...
<table>
    ...
    <tr>
      ...
      <td width="280px" valign="top">
        <sub>
          - Phaser 3 기반 미술 테마 화면과 자유 그림·색칠하기·AI 그림 퀴즈·작품 앨범 전체 콘텐츠 플로우 구현<br>
          - MediaPipe HandLandmarker 기반 손 추적·핀치 제스처·포인터 스무딩을 적용한 비접촉 드로잉 및 색칠 입력 시스템 개발<br>
          - REST·STOMP 기반 실시간 캐치마인드 멀티플레이와 방 생성·입장, 스트로크 동기화, 정답·점수·라운드 상태 복구 로직 구현<br>
          - Phaser 기반 리듬게임의 BPM 차트·롱노트·콤보·판정 시스템과 MediaPipe 양손 손바닥 입력 및 YouTube 동적 차트 기능 개발<br>
          - MediaRecorder 기반 플레이 영상·썸네일 생성, S3 Presigned URL 업로드 및 음악 점수·정확도·랭킹·작품 결과 저장 연동<br>
          - React·React Query 기반 보호자 페이지 구축 및 활동 기록·수행 영상·감정 대화·에너지·또래 평균 데이터 조회·시각화<br>
          - React Three Fiber·Three.js 기반 GLB 캐릭터 뷰어와 Pose Replay·Mocap 재생·관절 가동 범위 분석 UI 구현<br>
          - FastAPI·Spring·React 기반 AI 주간 리포트 End-to-End 연동 및 SSE·LiveKit 기반 실시간 알림·화면 공유·PTT 음성 기능 구현
      </td>
    </tr>
